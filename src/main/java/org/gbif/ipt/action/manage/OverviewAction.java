...
public class OverviewAction extends ManagerBaseAction {
  ...
  protected String numberCert;
  protected String dateCert;
  ...
  /**
   * Return the PDF size file format.
   */
  public String getPdfFormattedSize() {
    return FileUtils.formatSize(resourceManager.getPdfSize(resource), 0);
  }
  
  public boolean isPdfFileExisting() {
	    return resourceManager.isPdfExisting(resource.getShortname());
  }
  ...
  public void prepare() {
    super.prepare();
    String cert=resource.getCertNameFile();
    long timeLong=Long.parseLong(cert, 16); 
	Date dateN = new Date();
	dateN.setTime(timeLong);
	SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
	setDateCert(dateFormat.format(dateN));
    setNumberCert(cert);
    ...
    if (resource != null) {
      ...
      if(!emlValidator.isValidGen(resource.getEml(), "basic")){
    	  
    	  missingMetadata = true;
      }
      
      if(!emlValidator.isValidGen(resource.getEml(), "additional")){
    	  
    	  missingMetadata = true;
      }
      
      if(!emlValidator.isValidGen(resource.getEml(), "geocoverage")){
    	  
    	  missingMetadata = true;
      }
      
      
      if(!emlValidator.isValidGen(resource.getEml(), "taxcoverage")){
    	  
    	  missingMetadata = true;
      }
      
      if(!emlValidator.isValidGen(resource.getEml(), "tempcoverage")){
    	  
    	  missingMetadata = true;
      }
      
      if(!emlValidator.isValidGen(resource.getEml(), "methods")){
  ...
  public String publish() throws Exception {
    ...
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    String dateHex=Long.toHexString(curTime).toUpperCase();
    String nameCert=dateHex+"-"+dateFormat.format(date);
    resource.setCertNameFile(dateHex);
    try {
      // publish a new version of the resource
      if (resourceManager.publish(resource, v, nameCert,"es", this)) {
