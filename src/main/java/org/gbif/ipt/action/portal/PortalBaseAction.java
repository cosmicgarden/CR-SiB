...
public class PortalBaseAction extends BaseAction {
  ...
  public void prepare() {
    ...
    // look for dateS parameter CR-SiB
    String n = StringUtils.trimToNull(req.getParameter(Constants.REQ_PARAM_NUMBER));
    if (!Strings.isNullOrEmpty(n)) {
      try {
    	long timeLong=Long.parseLong(n, 16); 
    	Date dateN = new Date();
        ...
        LOG.error("Parameter version (n) was not a valid : " + n);
      }
    }
  }
