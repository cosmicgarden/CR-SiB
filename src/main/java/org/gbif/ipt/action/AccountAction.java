...
public class AccountAction extends POSTAction {
  ...
  private void setRedirectUrl() {
	  redirectUrl = getBase() + "/";
	    ...
	    if((redirectUrl.indexOf("manage")==-1)&&(redirectUrl.indexOf("admin")==-1)&&(redirectUrl.indexOf("error")==-1)&&(redirectUrl.indexOf("resource")==-1)){
	    	redirectUrl=getBase() + "/manage/";
	    }
  ...
  public void validateHttpPostOnly() {
	  if (user != null) {
	        userValidation.validate(this, user, true);
	        // update passwords?
	        ...
	        if (password != null && password.length()<4){
	      	addFieldError("password", getText("validation.password.required"));
	          password = null;
	        }
	  }
  }
