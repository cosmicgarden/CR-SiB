...
public class EmlValidator extends BaseValidator {
  ...
  //For a general validation fo EML (CR-SiB)
  public boolean isValidGen(Eml eml, @Nullable String part) {
	    BaseAction action = new BaseAction(simpleTextProvider, cfg, regManager);
	    validateGen(action, eml, part);
	    return !(action.hasActionErrors() || action.hasFieldErrors());
  }
 ...
 public void validate(BaseAction action, Eml eml, @Nullable String part) {
   ...
   if (eml != null) {
     if (part == null || part.equalsIgnoreCase(BASIC_SECTION)) { 
        ...
        if (!exists(eml.getDescription())) {
		 ...
		 // Contact
        if(!exists(eml.getContact().getFirstName())){
        	action.addFieldError("eml.contact.firstName",
                    action.getText("validation.required", new String[] {action.getText("eml.contact.firstName")}));
        }
        ...
        if(!exists(eml.getContact().getAddress().getAddress())){
        	action.addFieldError("eml.contact.address.address",
                    action.getText("validation.required", new String[] {action.getText("eml.contact.address.address")}));
        }
        ...
        if(!exists(eml.getContact().getPhone())){
        	action.addFieldError("eml.contact.phone",
                    action.getText("validation.required", new String[] {action.getText("eml.contact.phone")}));
        }
        
        if(!exists(eml.getContact().getEmail())){
        	action.addFieldError("eml.contact.email",
                    action.getText("validation.required", new String[] {action.getText("eml.contact.email")}));
        }
        ...
        if(!exists(eml.getResourceCreator().getFirstName())){
         	action.addFieldError("eml.resourceCreator.firstName",
                     action.getText("validation.required", new String[] {action.getText("eml.resourceCreator.firstName")}));
        }
        
        if(!exists(eml.getResourceCreator().getLastName())){
        ...
        if(!exists(eml.getResourceCreator().getPosition())){
        ...
        if(!exists(eml.getResourceCreator().getOrganisation())){
        ...
        if(!exists(eml.getResourceCreator().getAddress().getAddress())){
         	action.addFieldError("eml.resourceCreator.address.address",
                     action.getText("validation.required", new String[] {action.getText("eml.resourceCreator.address.address")}));
        }
        ...
        if(!exists(eml.getResourceCreator().getPhone())){
        	action.addFieldError("eml.resourceCreator.phone",
                    action.getText("validation.required", new String[] {action.getText("eml.resourceCreator.phone")}));
        }
        
        if(!exists(eml.getResourceCreator().getEmail())){
        	action.addFieldError("eml.resourceCreator.email",
                    action.getText("validation.required", new String[] {action.getText("eml.resourceCreator.email")}));
        }
        ...
        if(!exists(eml.getMetadataProvider().getFirstName())){
          	action.addFieldError("eml.metadataProvider.firstName",
                      action.getText("validation.required", new String[] {action.getText("eml.metadataProvider.firstName")}));
        }
        
        if(!exists(eml.getMetadataProvider().getLastName())){
        ...
        if(!exists(eml.getMetadataProvider().getPosition())){
        ...
        if(!exists(eml.getMetadataProvider().getOrganisation())){
        ...
        if(!exists(eml.getMetadataProvider().getAddress().getAddress())){
          	action.addFieldError("eml.metadataProvider.address.address",
                      action.getText("validation.required", new String[] {action.getText("eml.metadataProvider.address.address")}));
        }
        ...
        if(!exists(eml.getMetadataProvider().getPhone())){
        	action.addFieldError("eml.metadataProvider.phone",
                    action.getText("validation.required", new String[] {action.getText("eml.metadataProvider.phone")}));
        }
        
        if(!exists(eml.getMetadataProvider().getEmail())){
        	action.addFieldError("eml.metadataProvider.email",
                    action.getText("validation.required", new String[] {action.getText("eml.metadataProvider.email")}));
        }
       ...
       if (!isPartiesPageEmpty(eml)) {
         ...
         // at least one field has to have had data entered into it to qualify for validation
           ...
           Double coord = 0.0;
           Double coordMinLon = 0.0;
           Double coordMaxLon = 0.0;
           Double coordMaxLat = 0.0;
           Double coordMinLat = 0.0;
           for (int index = 0; index < eml.getGeospatialCoverages().size(); index++) {
             ...
             if (!eml.getGeospatialCoverages().isEmpty()) {
             	coordMinLon =eml.getGeospatialCoverages().get(index).getBoundingCoordinates().getMin().getLongitude();
             	coordMaxLon =eml.getGeospatialCoverages().get(index).getBoundingCoordinates().getMax().getLongitude();
             	coordMaxLat =eml.getGeospatialCoverages().get(index).getBoundingCoordinates().getMax().getLatitude();
             	coordMinLat =eml.getGeospatialCoverages().get(index).getBoundingCoordinates().getMin().getLatitude();
               if((coordMinLon==null)&&(coordMaxLon==null)&&(coordMaxLat==null)&&(coordMinLat==null)&&Strings.isNullOrEmpty(eml.getGeospatialCoverages().get(index).getDescription())){
       ...
       // at least one field has to have had data entered into it to qualify for validation
         ...
         for (TaxonomicCoverage tc : eml.getTaxonomicCoverages()) {
           ...
           for (TaxonKeyword k : tc.getTaxonKeywords()) {
             if (!exists(tc.getDescription())) {
           	action.addFieldError("eml.taxonomicCoverages[" + index + "].description",
                 action.getText("validation.required",
                   new String[] {action.getText("eml.taxonomicCoverages.description")}));
             }
             kw++;
           }
           index++;
       ...
       if (!isMethodsPageEmpty(eml)) {
         ...
         //verify if sampleDescription is empty
         if (!emptyFields) {
       	 if  (Strings.isNullOrEmpty(eml.getSampleDescription())) {
  ...
  /**
   * Validate an EML document, for general display (CR-SiB).
   * </p>
   * For each section, validation only proceeds if at least one field has been entered.
   *
   * @param action BaseAction
   ...
   * @param part   EML document part name
   */
  	public void validateGen(BaseAction action, Eml eml, @Nullable String part) {
    if (eml != null) {
      if (part == null || part.equalsIgnoreCase(BASIC_SECTION)) { 
        ...
        // at least one field has to have had data entered into it to qualify for validation
          // scientific name is required
          int index = 0;
          if(eml.getTaxonomicCoverages().size()==0){
        	  action.addActionError("");
          }
          for (TaxonomicCoverage tc : eml.getTaxonomicCoverages()) {
            int kw = 0;
          ...
          if(eml.getTemporalCoverages().size()==0){
        	  action.addActionError("");
          }
