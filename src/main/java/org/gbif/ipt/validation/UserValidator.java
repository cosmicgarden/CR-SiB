...
public class UserValidator extends BaseValidator {
  ...
  public boolean validate(BaseAction action, User user, boolean newuser) {
    boolean valid = true;
    if (user != null) {
      ...
      //New validatiosn CR-SiB
      if (!exists(user.getIdentification(), 1)) {
          valid = false;
          action.addFieldError("user.identification", action.getText("validation.identification.required"));
      }
      
      /*
      if (!exists(user.getInstitution(), 1)) {
          valid = false;
          action.addFieldError("user.institution", action.getText("validation.identification.required"));
      }
      */
      if(newuser){
