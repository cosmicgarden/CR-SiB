...
public class DataDir {
   ...
   * @return latest published version of RTF file
   */
  public File resourcePdfFile(String resourceName) {
    String fn = resourceName + ".pdf";
    return dataFile(RESOURCES_DIR + "/" + resourceName + "/" + fn);
  }
  
  /**
   * Retrieves file for the latest published version of the PDF file representing the EML metadata in PDF format.
   * Specific versions can also be resolved depending on the parameter "version". If no specific version is
   ...
   * @param version      version
   *
   * @return PDF file having specific version, defaulting to latest published version if no version specified
   */
  public File resourcePdfFile(String resourceName, @Nullable String nameCert) {
    String fn;
    if (nameCert == null) {
      fn = resourceName + ".pdf";
    } else {
      fn = resourceName + "-" + nameCert + ".pdf";
    }
    return dataFile(RESOURCES_DIR + "/" + resourceName + "/" + fn);
  }
