...
<script type="text/javascript" charset="utf-8">
    ...
    var arrCompl = new Array();

    <#if cond??>
        <#list cond as cp>
        if('${cp}'=='P'){
          arrCompl.push('En proceso');
        }else if('${cp}'=='R'){
          arrCompl.push('Reportado');
        }
        </#list>
    </#if>
    ...
    var aDataSet = [
      <#list resources as r>
          ...
          <#if cond??>arrCompl[${r_index}]<#else>'${emptyString}'</#if>,
          <#if r.eml.additionalInfo?has_content>arrData[${r_index}][1]<#else>'${emptyString}'</#if>]<#if r_has_next>,</#if>
      </#list>
    ];

    $(document).ready(function() {
        $('#rtableContainer').html( '<table cellpadding="3" cellspacing="3" border="0" class="display" id="rtableMan"></table>' );
        $('#rtableMan').dataTable( {
            "aaData": aDataSet,
            ...
            "aoColumns": [
                { "sTitle": "<@s.text name="dataTables.manage.name"/>"},
                { "sTitle": "<@s.text name="dataTables.manage.modified" />"},
                { "sTitle": "<@s.text name="dataTables.manage.certidate" />"},
                { "sTitle": "<@s.text name="dataTables.manage.state" />"},
                { "sTitle": "<@s.text name="dataTables.manage.numlinc" />"}
            ]
        } );
        $('#rtableMan').dataTable().columnFilter({ sPlaceHolder: "head:after",
                                                    aoColumns: [
                                                                { type: "text" },
                                                                { type: "text" },
                                                                { type: "text" },
