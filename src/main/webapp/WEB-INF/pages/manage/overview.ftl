...
$(document).ready(function(){
  ...
  $('#publishButton').jConfirmAction({question : "<@s.text name='report.confirm'/>", yesAnswer : "<@s.text name='report.yes'/>", cancelAnswer : "<@s.text name='report.no'/>"});
...
console.log('${resource.emlVersion}');
</script>
...
<div class="resourceOverview" id="publish">
  ...
  <img src="${baseURL}/images/icons/medal_ico.png"/>
    <div class="head">
    ...
    <#else>
      ...
      <table>
        <#if resource.lastPublished?? >
          ...
          <tr>
            <a class="imglink" href="${baseURL}/pdf.do?r=${resource.shortname}&n=<#if numberCert??>${numberCert}<#else>"--"</#if>" onClick="_gaq.push(['_trackEvent', 'PDF', 'Download', '${resource.shortname}']);"><img src="${baseURL}/images/icons/ico_reporte.png" /><@s.text name='portal.resource.download_pdf'/></a>
            <th><@s.text name='Número del certificado'/></th><td><#if numberCert??>${numberCert}<#else>"--"</#if></td>
...
<#if currentUser.getRole()=="Admin" || resource.emlVersion==0 >
<div>
