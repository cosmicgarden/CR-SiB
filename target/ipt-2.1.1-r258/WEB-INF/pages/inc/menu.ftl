          ...
          <div class="grid_13 region region-menu" id="region-menu">
            <nav>
              <ul>
                ...
                [#if managerRights]
                <li[#if currentMenu=='manage'] class="current"[/#if]><a href="${baseURL}/manage/"><img src="${baseURL}/images/icons/manage.png">[@s.text name="menu.manage"/]</a></li>
                [/#if]
                [#if adminRights]
...
[#if sideMenuEml!false]
      ...
      <div class="clearfix" id="sidebar">
        <h2>[@s.text name='manage.metadata.required.section' /]</h2>
        <ul class="sidebar">
        [#list ["basic", "additional", "geocoverage", "taxcoverage","tempcoverage", "methods", "collections"] as it]
        ...
        <h2>[@s.text name='manage.metadata.optional.section' /]</h2>
        <ul class="sidebar">
        [#list ["project","keywords", "parties"] as it]
         <li[#if currentSideMenu?? && currentSideMenu==it] class="current"[#else] class="sidebar"[/#if]><a href="metadata-${it}.do?r=${resource.shortname!r!}">[@s.text name="submenu.${it}"/]</a></li>
        [/#list]
        </ul>
        <ul class="sidebar">
          <h2><a class="overlink" href="resource.do?r=${resource.shortname}">[@s.text name='manage.metadata.overview' /]</a></h2>
        </ul>
      </div>
      </aside>
