...
<span class="small">${now?datetime?string("yyyy-MM-dd")}</span>
...
<#if report??>
  ...
  <#if report.completed>
    ...
    <p><a href="${baseURL}/pdf.do?r=${resource.shortname}&n=<#if numberCert??>${numberCert}<#else>"--"</#if>" onClick="_gaq.push(['_trackEvent', 'PDF', 'Download', '${resource.shortname}']);"><img src="${baseURL}/images/icons/ico_reporte.png" /><@s.text name='portal.resource.download_pdf'/></a></p>
